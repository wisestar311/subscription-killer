...
4. 앱 설정 화면에 표시된 URL과 Authorization 헤더를 복사합니다.
5. 메서드를 `POST`, 요청 본문을 `파일`로 설정하고 값에는 수신한 메시지인 `단축어 입력`을 지정합니다.

기존 JSON 방식도 계속 지원합니다. JSON 본문을 사용할 때는 `token`과 `message` 값을 전송합니다.
